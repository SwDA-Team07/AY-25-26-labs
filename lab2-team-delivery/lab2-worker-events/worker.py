...
def ack_log(reason: str) -> None:
    log(f"ack: {reason}")


async def ack_message(message: aio_pika.IncomingMessage, reason: str) -> None:
    await message.ack()
    ack_log(reason)
...
async def reject_message(
    ...
    *,
    ...
    log(f"reject: {reason} (requeue={str(requeue).lower()})")
...
async def handle_message(
    ...
    try:
        ...
        await reject_message(message, "invalid json", requeue=False)
        return

    data = payload.get("data") if isinstance(payload, dict) else None
    if not isinstance(data, dict):
        log("payload missing 'data', dropping message")
        await reject_message(message, "missing data", requeue=False)
        return
    ...
    if operation == "update":
        ...
        await ack_message(message, "update event ignored")
        return

    if not communication_id:
        log("event missing doc.id, dropping message")
        await reject_message(message, "missing communication id", requeue=False)
        return
    ...
    try:
        ...
        log(f"{communication_id} fetch failed: {err}")
        await reject_message(message, f"{communication_id} fetch failed", requeue=True)
        return

    if not full_document:
        log(f"{communication_id} not found, skipping")
        await ack_message(message, "communication not found")
        return
    ...
    if current_status in {"sent", "processing"}:
        ...
        await ack_message(message, f"{communication_id} already {current_status}")
        return
    ...
    try:
        ...
        log(f"{communication_id} could not be claimed: {err}")
        await reject_message(message, f"{communication_id} claim failed", requeue=True)
        return
    ...
    try:
        ...
        try:
            ...
            log(f"{communication_id} could not be marked failed: {patch_err}")
            await reject_message(
                message,
                f"{communication_id} failed status patch failed",
                requeue=True,
            )
            return

        log(f"{communication_id} set to failed")
        await ack_message(message, f"{communication_id} failed status persisted")
        return

    await ack_message(message, f"{communication_id} processing completed")
